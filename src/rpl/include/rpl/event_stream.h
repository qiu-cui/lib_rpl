...
namespace rpl {
	...
	public:
		...
		lifetime(const lifetime& o) : composite_subscription(o) {}
		lifetime(lifetime&& o) : composite_subscription(std::move(o)) {}
		lifetime& operator=(lifetime o) {
			composite_subscription::operator=(std::move(o));
			return *this;
		}
		~lifetime() {
			unsubscribe();
	...
	using producer = rxcpp::observable<Value>;
	...
	public:
		...
		void fire_forward(Value&& value) const {
			...
			if (const auto strong = weak.lock()) {
				strong->get_subscriber().on_next(std::move(value));
			}
		}
		...
		void fire_error_forward(std::exception_ptr &&error) const {
			auto weak = make_weak();
			if (const auto strong = weak.lock()) {
				strong->get_subscriber().on_error(std::move(error));
			}
		}
        void fire_error(std::exception_ptr &&error) const {
			return fire_error_forward(std::move(error));
		}
        void fire_error_copy(const std::exception_ptr &error) const {
		...
		auto events_starting_with(Value && value) const {
			return events().start_with(std::move(value));
		}
		auto events_starting_with_copy(const Value & value) const {
			return events().start_with(value);
		}
		bool has_consumers() const {
			return (_data != nullptr) && _data->has_observers();
		}
	...
	void event_stream<Value>::fire_done() const {
		if (const auto data = std::exchange(_data, nullptr)) {
			data->get_subscription().unsubscribe();
		}
	}

	template <typename Value>
	inline auto event_stream<Value>::make_weak() const
		-> std::weak_ptr<rxcpp::subjects::subject<Value>> {
		if (!_data) {
			_data = std::make_shared<rxcpp::subjects::subject<Value>>();
		}
		return _data;
	}
		...
		return [&](auto&& observable) {
			observable.subscribe(
                ...
                [&](std::exception_ptr error) {
					stream.fire_error(std::move(error));
				},
				[&]() {
					stream.fire_done();
				});
	...
	namespace details {
		...
		public:
			explicit start_spawning_helper(lifetime& alive_while)
				: _lifetime(alive_while) {}

			template <typename Value>
			auto operator()(rxcpp::observable<Value>&& initial)
				...
				auto collecting = stream->events().subscribe(
				...
				if (maybeError.has_value()) {
					return rxcpp::observable<>::create<Value>(
						[error = std::move(*maybeError)](rxcpp::subscriber<Value> s) {
							s.on_error(std::move(error));
						});
				}

				return rxcpp::observable<>::iterate(std::move(values))
					.concat(stream->events());
			}
